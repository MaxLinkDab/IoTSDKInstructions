...
public class MoneyUtil {
    ...
    public static String moneyMul(String valueStr,String mulStr){
        if(valueStr.equals("null") || mulStr.equals("null") || valueStr.equals("0") || mulStr.equals("0") || null == valueStr ||null == mulStr){
            return "0";
        }
        BigDecimal value = new BigDecimal(valueStr);
    ...
    public static BigDecimal moneyMul(BigDecimal value,BigDecimal mulValue){
        if(null == value || null ==mulValue || value.compareTo(BigDecimal.ZERO)==0 ||mulValue.compareTo(BigDecimal.ZERO)==0){
            return BigDecimal.ZERO;
        }
        return (value.multiply(mulValue)).setScale(9, BigDecimal.ROUND_DOWN);
    }
    ...
    public static String moneydiv(String valueStr,String divideStr){
        if(valueStr.equals("null") || divideStr.equals("null") || valueStr.equals("0") || divideStr.equals("0") || null == valueStr ||null == divideStr){
            return "0";
        }
        BigDecimal value = new BigDecimal(valueStr);
        BigDecimal divideValue = new BigDecimal(divideStr);
        return fnum.format(value.divide(divideValue, 9, BigDecimal.ROUND_DOWN));
    }
    ...
    public static BigDecimal moneydiv(BigDecimal value,BigDecimal divideValue){
        if(null == value || null ==divideValue || divideValue.compareTo(BigDecimal.ZERO)==0 || value.compareTo(BigDecimal.ZERO)==0){
            return BigDecimal.ZERO;
        }else{
            return value.divide(divideValue, 9, BigDecimal.ROUND_DOWN);
        }
    }
